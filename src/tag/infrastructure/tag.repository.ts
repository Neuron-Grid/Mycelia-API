...
export class TagRepository {
    ...
    async create(
        ...
        const insertData: TagsInsert = {
            ...
            tag_emb: data.tag_embedding ? JSON.stringify(data.tag_embedding) : null,
        }
    ...
    async update(
        ...
        if (data.parent_tag_id !== undefined) updateData.parent_tag_id = data.parent_tag_id
        if (data.description !== undefined) updateData.description = data.description
        if (data.color !== undefined) updateData.color = data.color
        if (data.tag_embedding !== undefined) {
            updateData.tag_emb = data.tag_embedding ? JSON.stringify(data.tag_embedding) : null
        }

        // parent_tag_idの所有者確認
        if (updateData.parent_tag_id !== undefined && updateData.parent_tag_id !== null) {
            const { data: parent, error: pe } = await supabase
                .from('tags')
                .select('id')
    ...
    async findByNameAndParent(
        ...
        if (error) {
            ...
            this.logger.error(`findByNameAndParent failed: ${error.message}`, error)
            throw error
        }
        return new TagEntity(data)
    }
    ...
    async tagSubscription(userId: string, subscriptionId: number, tagIds: number[]): Promise<void> {
        ...
        if (error) {
            this.logger.error(`tagSubscription failed: ${error.message}`, error)
            throw error
        }
    }
    ...
    async getFeedItemsByTags(userId: string, tagIds: number[]) {
        ...
        if (error) {
            this.logger.error(`getFeedItemsByTags failed: ${error.message}`, error)
            throw error
        }
        return data ?? []
    }
    ...
    async getSubscriptionsByTags(userId: string, tagIds: number[]) {
        ...
        if (error) {
            this.logger.error(`getSubscriptionsByTags failed: ${error.message}`, error)
            throw error
        }
        return data ?? []
    }
