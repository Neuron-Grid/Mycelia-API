...
export class MockLlmService implements LlmService {
    ...
    async generateSummary(request: GeminiSummaryRequest): Promise<GeminiSummaryResponse> {
        ...
        this.logger.debug(`Generating mock summary for articles count: ${request.articles.length}, target language: ${request.targetLanguage || 'auto'}`);
    ...
    async generateScript(request: GeminiScriptRequest): Promise<GeminiScriptResponse> {
        ...
        this.logger.debug(`Generating mock script based on summary: "${request.summaryText.substring(0, 30)}..."`);
