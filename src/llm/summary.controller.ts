...
export class SummaryController {
    ...
    @ApiOperation({ summary: 'Regenerate summary for a user (typically for today or a specific date)' })
    @ApiParam({ name: 'userId', description: 'User ID for whom to regenerate summary', type: 'string' })
    @ApiBearerAuth()
    ...
    ): Promise<{ message: string; jobId?: string }> { // 戻り値の型を明確化
        this.logger.log(`User ${requestingUser.id} requesting summary regeneration for user ${targetUserId}`);

        // 管理者ユーザーは不要なので、リクエスト発行者が対象ユーザーと一致する場合のみ許可
        if (requestingUser.id !== targetUserId) {
            this.logger.warn(`Forbidden: User ${requestingUser.id} attempted to regenerate summary for ${targetUserId}`);
            throw new HttpException('Forbidden: You can only regenerate your own summaries.', HttpStatus.FORBIDDEN);
        }

        try {
            // SummaryScriptServiceのメソッドはBullMQのaddを呼び出し、Jobオブジェクト(またはその一部)を返す想定
            // Jobオブジェクトにはidが含まれる
            const result = await this.summaryScriptService.requestSummaryGeneration(targetUserId, body?.prompt); // await を追加
            ...
            this.logger.error(`Failed to queue summary regeneration for user ${targetUserId}: ${errorMessage}`, error instanceof Error ? error.stack : undefined);
            throw new HttpException('Failed to queue summary regeneration.', HttpStatus.INTERNAL_SERVER_ERROR);
        }
    }
    ...
    @ApiParam({ name: 'summaryId', description: 'ID of the daily_summary to regenerate script for', type: 'number' })
    @ApiBearerAuth()
    @UseGuards(SupabaseAuthGuard)
    async regenerateScript(
        ...
        this.logger.log(`User ${user.id} requesting script regeneration for summary ID ${summaryId}`);
        ...
        */
        ...
        this.logger.warn(`Ownership check for summaryId ${summaryId} by user ${user.id} is currently skipped. Implement this check!`);


        try {
            const result = await this.summaryScriptService.requestScriptGeneration(summaryId, body?.prompt); // await を追加
            const message = `Script regeneration job (ID: ${result.jobId}) has been queued for summary ID ${summaryId}.`;
            this.logger.log(message);
            return { message, jobId: result.jobId };
        } catch (error: unknown) {
            const errorMessage = error instanceof Error ? error.message : 'Unknown error occurred';
            this.logger.error(`Failed to queue script regeneration for summary ID ${summaryId}: ${errorMessage}`, error instanceof Error ? error.stack : undefined);
            throw new HttpException('Failed to queue script regeneration.', HttpStatus.INTERNAL_SERVER_ERROR);
        }
    }
}