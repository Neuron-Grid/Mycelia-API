// src/llm/gemini-flash.client.ts
...
interface GeminiErrorDetail { // Gemini APIが返す可能性のあるエラー詳細構造
    '@type'?: string;
...
interface GeminiErrorPayload { // Gemini APIの実際のエラーレスポンスの'error'フィールド
...
export class GeminiFlashClient implements LlmService {
    ...
    private async makeApiCall<TRequest extends object, TResponse>( // TRequest に object 制約を追加
        requestPayload: TRequest,
    ): Promise<AxiosResponse<TResponse>> { // この関数は await を使うので async のまま
        // `backOff` は Promise を返すので、`await` が必要
        return await backOff( // 修正: await を追加 (lint/suspicious/useAwait 関連)
            async () => { // backOffのコールバックもasyncにする
                return await firstValueFrom( // 修正: await を追加
                    this.http.post<TResponse>(
                        `${this.apiUrl}?key=${this.apiKey}`, // 修正: useTemplate テンプレートリテラル使用
                        requestPayload,
            ...
            },
            {
                ...
                startingDelay: 1000,
                timeMultiple: 3,
                retry: (e: unknown, attemptNumber: number) => { // 修正: e の型を unknown に (lint/suspicious/noExplicitAny 関連)
                    ...
                    if (this.isAxiosError(e)) {
                        ...
                        if (status === 429 || geminiErrorStatus === 'RESOURCE_EXHAUSTED') {
                            this.logger.warn(`Retrying due to 429/RESOURCE_EXHAUSTED. Attempt: ${attemptNumber}`);
                            return true;
                        }
                        if (status === 400) {
                            this.logger.error(`Non-retriable error 400: ${e.response?.data?.message || e.message}`);
                            return false;
                        }
                        if (status && status >= 500) {
                            this.logger.warn(`Retrying due to 5xx error. Status: ${status}. Attempt: ${attemptNumber}`);
                            return true;
                        }
                    }
                    this.logger.error(`Unknown or non-retriable error during API call: ${(e instanceof Error) ? e.message : String(e)}`);
                    return false;
                },
            },
    ...
    public async generateSummary(request: GeminiSummaryRequest): Promise<GeminiSummaryResponse> {
        ...
        const langInstruction = request.targetLanguage === 'ja'
            ? '日本語で記述してください。'
            : '英語で記述してください。';
        ...
        try {
            ...
            if (responseText.length > 7000) {
                this.logger.warn(`Summary length ${responseText.length} exceeds 7000 chars, truncating.`);
                responseText = `${responseText.substring(0, 7000)}... [truncated]`; // 修正: useTemplate
            }

            return { content: responseText };
        } catch (error: unknown) { // 修正: noExplicitAny
    ...
    public async generateScript(request: GeminiScriptRequest): Promise<GeminiScriptResponse> {
        const systemInstruction = "あなたはプロのニュースキャスターです。自然で聞き取りやすい日本語のナレーション原稿を作成してください。";
        const articlesJsonString = request.articlesForContext
            ? `関連ニュース記事の概要JSON: ${JSON.stringify(request.articlesForContext.map(a => ({ title: sanitizeMarkdown(a.title), url: a.url })))}` // 修正: useTemplate
            : '';
