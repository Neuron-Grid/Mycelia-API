...
describe("EmbeddingBatchDataService", () => {
    ...
    it("returns a single feed item with combined text", async () => {
        const row: Database["public"]["Functions"]["fn_list_missing_feed_item_embeddings"]["Returns"][number] =
            {
                id: 1,
                title: "Title",
                description: "Desc",
    ...
    it("returns a single daily summary using markdown or title", async () => {
        ...
        } as Database["public"]["Functions"]["fn_find_daily_summary_by_id"]["Returns"][number];
        client.rpc.mockResolvedValue({ data: [row], error: null });
    ...
    it("returns a single podcast episode title", async () => {
        const row = {
            id: 3,
            title: "Episode",
        } as Database["public"]["Functions"]["fn_find_podcast_by_id"]["Returns"][number];
        client.rpc.mockResolvedValue({ data: [row], error: null });
    ...
    it("returns a single tag with description", async () => {
        const row: Database["public"]["Functions"]["fn_list_missing_tag_embeddings"]["Returns"][number] =
            {
                id: 4,
                tag_name: "Tag",
                description: "Note",
