...
export class SearchController {
    ...
    @ApiOperation({ 
        ...
        description: 'Search across feed items, summaries, and podcast episodes using vector similarity'
    })
    @ApiResponse({ status: 200, description: 'Search results returned successfully' })
    @ApiQuery({ name: 'q', description: 'Search query', required: true })
    @ApiQuery({ name: 'limit', description: 'Maximum number of results (default: 20)', required: false })
    @ApiQuery({ name: 'threshold', description: 'Similarity threshold 0.0-1.0 (default: 0.7)', required: false })
    @ApiQuery({ 
        name: 'types', 
    ...
    async searchAll(
        ...
        if (types) {
            const typeArray = types.split(',').map(t => t.trim()) as ('feed_item' | 'summary' | 'podcast')[]
            options.includeTypes = typeArray.filter(t => ['feed_item', 'summary', 'podcast'].includes(t))
        }
    ...
    async searchPodcasts(
        ...
        const results = await this.vectorSearchService.searchPodcastEpisodes(user.id, query, options)
