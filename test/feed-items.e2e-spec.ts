...
describe("feed_items link_hash trigger (e2e)", () => {
    ...
    const serviceRoleKey = config.get<string>(
    ...
    beforeAll(async () => {
        ...
        anonClient = createClient<Database>(resolvedSupabaseUrl, resolvedAnonKey, {
            auth: { autoRefreshToken: false, persistSession: false },
        });
        ...
        if (createUserError || !userData?.user?.id) {
            throw createUserError ?? new Error("ユーザー作成に失敗しました");
        }
        testUserId = userData.user.id;

        const { data: primarySub, error: primarySubError } = await adminClient
        ...
        if (primarySubError || !primarySub?.id) {
            throw primarySubError ?? new Error("user_subscriptions作成に失敗しました");
        }
        primarySubscriptionId = primarySub.id;

        const { data: secondarySub, error: secondarySubError } = await adminClient
            .from("user_subscriptions")
            .insert({
                user_id: testUserId,
        ...
        if (signInError || !signInData.session?.access_token) {
            throw signInError ?? new Error("テストユーザーのサインインに失敗しました");
        }
    ...
    const fetchByTitle = async (
        ...
        const { data, error } = await client
            ...
            .eq("title", title)
            .single();
        if (error || !data) throw error ?? new Error("feed_items取得に失敗");
        return data;
    };
    ...
    it("Case A: canonical_url未指定でもトリガーでハッシュが付与される", async () => {
        ...
        await expect(insertFeedItemOrThrow(repository, payload)).resolves.toBeUndefined();
    ...
    it("Case B: canonical_urlが優先され正規化結果でハッシュが固定される", async () => {
        ...
        const row = await fetchByTitle(client, title);
        expect(row.link_hash).toBe(
            computeExpectedHash(canonicalRaw, link),
        );
    });
    ...
    it("Case C: 同一subscriptionで正規化後URLが一致すると23505で弾かれる", async () => {
        ...
        await expect(insertFeedItemOrThrow(repository, payload)).rejects.toMatchObject({
    ...
    it("Case D: 並列挿入でも1件のみ成功し一意制約が動作する", async () => {
        ...
        const code = (rejection.reason?.code ?? rejection.reason?.details?.code ?? "") as string;
    ...
    it("Case E: UPDATEでlink_hashが再計算され重複時は23505", async () => {
        ...
        if (updateError || !updatedRows) {
            throw updateError ?? new Error("feed_itemsの更新に失敗しました");
        }
